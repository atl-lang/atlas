...
impl Inspector {
    ...
    pub fn evaluate_expression(&self, expression: &str, variables: &[Variable]) -> EvalResult {
        ...
        // D-052: unified Compiler + VM execution path
        ...
        let bytecode = match compiler.compile(&ast) {
            Ok(bc) => bc,
            Err(diags) => {
                return EvalResult::Error(format!("compile error: {:?}", diags[0]));
            }
        };
