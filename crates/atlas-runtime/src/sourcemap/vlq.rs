...
mod tests {
    ...
    fn test_encode_larger_values() {
        // 16 → encoded as 32 (shifted) → needs 2 VLQ digits
    ...
    fn test_roundtrip_positive() {
        for v in [0, 1, 5, 15, 16, 100, 1000, 65535] {
    ...
    fn test_decode_segment() {
        let segment = encode_segment(&[0, 0, 0, 0]);
        let decoded = decode_segment(&segment).unwrap();
        assert_eq!(decoded, vec![0, 0, 0, 0]);
    }

    #[test]
    fn test_encode_segment_complex() {
        let values = [5, -3, 10, 0];
        let segment = encode_segment(&values);
        let decoded = decode_segment(&segment).unwrap();
    ...
    #[test]
    fn test_decode_segment_empty() {
        let decoded = decode_segment("").unwrap();
