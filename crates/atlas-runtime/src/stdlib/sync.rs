...
pub fn rwlock_read(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    ...
    let lock = rwlocks().lock().unwrap().get(&id).cloned().ok_or_else(|| RuntimeError::InvalidStdlibArgument {
        msg: "rwLockRead(): handle has been destroyed".into(),
        span,
    })?;
    let guard = lock.read().map_err(|e| RuntimeError::InvalidStdlibArgument {
...
pub fn rwlock_write(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    if args.len() != 2 {
        return Err(super::stdlib_arity_error("rwLockWrite", 2, args.len(), span));
    }
    ...
    let mut guard = lock.write().map_err(|e| RuntimeError::InvalidStdlibArgument {
...
pub fn rwlock_try_read(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    if args.len() != 1 {
        return Err(super::stdlib_arity_error("rwLockTryRead", 1, args.len(), span));
    }
...
pub fn rwlock_try_write(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    if args.len() != 2 {
        return Err(super::stdlib_arity_error("rwLockTryWrite", 2, args.len(), span));
    }
...
pub fn semaphore_new(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    if args.len() != 1 {
        return Err(super::stdlib_arity_error("semaphoreNew", 1, args.len(), span));
    }
    let permits = match &args[0] {
        ...
        _ => return Err(super::stdlib_arg_error("semaphoreNew", "number", &args[0], span)),
    };
...
pub fn semaphore_acquire(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    if args.len() != 1 {
        return Err(super::stdlib_arity_error("semaphoreAcquire", 1, args.len(), span));
    }
    let id = extract_handle_id(&args[0], SEMAPHORE_TAG, "semaphoreAcquire", span)?;
    let sem = semaphores().lock().unwrap().get(&id).cloned().ok_or_else(|| RuntimeError::InvalidStdlibArgument {
...
pub fn semaphore_try_acquire(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    if args.len() != 1 {
        return Err(super::stdlib_arity_error("semaphoreTryAcquire", 1, args.len(), span));
    }
...
pub fn semaphore_release(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    if args.len() != 1 {
        return Err(super::stdlib_arity_error("semaphoreRelease", 1, args.len(), span));
    }
...
pub fn semaphore_available(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    if args.len() != 1 {
        return Err(super::stdlib_arity_error("semaphoreAvailable", 1, args.len(), span));
    }
...
pub fn atomic_new(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    ...
    let initial = match &args[0] {
        ...
        _ => return Err(super::stdlib_arg_error("atomicNew", "number", &args[0], span)),
    };
...
pub fn atomic_load(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    ...
    let atomic = atomics().lock().unwrap().get(&id).cloned().ok_or_else(|| RuntimeError::InvalidStdlibArgument {
...
pub fn atomic_store(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    if args.len() != 2 {
        return Err(super::stdlib_arity_error("atomicStore", 2, args.len(), span));
    }
    let id = extract_handle_id(&args[0], ATOMIC_TAG, "atomicStore", span)?;
    let val = match &args[1] {
        Value::Number(n) => *n as i64,
        _ => return Err(super::stdlib_arg_error("atomicStore", "number", &args[1], span)),
    };
    let atomic = atomics().lock().unwrap().get(&id).cloned().ok_or_else(|| RuntimeError::InvalidStdlibArgument {
...
pub fn atomic_add(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    ...
    let delta = match &args[1] {
        Value::Number(n) => *n as i64,
        _ => return Err(super::stdlib_arg_error("atomicAdd", "number", &args[1], span)),
    };
    let atomic = atomics().lock().unwrap().get(&id).cloned().ok_or_else(|| RuntimeError::InvalidStdlibArgument {
...
pub fn atomic_sub(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    ...
    let delta = match &args[1] {
        Value::Number(n) => *n as i64,
        _ => return Err(super::stdlib_arg_error("atomicSub", "number", &args[1], span)),
    };
    let atomic = atomics().lock().unwrap().get(&id).cloned().ok_or_else(|| RuntimeError::InvalidStdlibArgument {
...
pub fn atomic_compare_exchange(args: &[Value], span: Span) -> Result<Value, RuntimeError> {
    if args.len() != 3 {
        return Err(super::stdlib_arity_error("atomicCompareExchange", 3, args.len(), span));
    }
    let id = extract_handle_id(&args[0], ATOMIC_TAG, "atomicCompareExchange", span)?;
    let expected = match &args[1] {
        Value::Number(n) => *n as i64,
        _ => return Err(super::stdlib_arg_error("atomicCompareExchange", "number", &args[1], span)),
    };
    let desired = match &args[2] {
        Value::Number(n) => *n as i64,
        _ => return Err(super::stdlib_arg_error("atomicCompareExchange", "number", &args[2], span)),
    };
    let atomic = atomics().lock().unwrap().get(&id).cloned().ok_or_else(|| RuntimeError::InvalidStdlibArgument {
