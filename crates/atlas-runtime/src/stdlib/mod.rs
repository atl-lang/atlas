...
fn stdlib_signature(func_name: &str) -> Option<&'static str> {
    match func_name {
        ...
        // Array functions
        // NOTE: arrayPush/arrayPop/etc. are INTERNAL routing names only.
        // User-facing syntax is arr.push(x), arr.pop(), etc. (method syntax).
        // These names appear here only to generate helpful error messages.
        // DO NOT document these as valid Atlas syntax.
        "arrayPush" => Some("arr.push(value: T): T[]  [use method syntax, not arrayPush()]"),
        "arrayPop" => Some("arr.pop(): [T, T[]]  [use method syntax, not arrayPop()]"),
        "arrayShift" => Some("arr.shift(): [T, T[]]  [use method syntax, not arrayShift()]"),
        "arrayUnshift" => {
            Some("arr.unshift(value: T): T[]  [use method syntax, not arrayUnshift()]")
        }
        "arraySlice" => Some("arr.slice(start: number, end: number): T[]  [use method syntax]"),
        "arrayConcat" => Some("arr.concat(other: T[]): T[]  [use method syntax]"),
        "arrayIncludes" => Some("arr.includes(value: T): bool  [use method syntax]"),
        "arrayIndexOf" => Some("arr.indexOf(value: T): Option<number>  [use method syntax]"),
        "arrayReverse" => Some("arr.reverse(): T[]  [use method syntax]"),
        "arraySort" => Some("arr.sort(): T[]  [use method syntax]"),
        "arrayEnumerate" => Some("arr.enumerate(): (number, T)[]  [use method syntax]"),
        "arrayFlat" => Some("arr.flat(): T[]  [use method syntax]"),
        "arrayFlatMap" => Some("arr.flatMap(fn: (T): U[]): U[]  [use method syntax]"),
        "arrayFill" => {
            Some("arr.fill(value: T, start: number, end: number): T[]  [use method syntax]")
        }
...
fn builtin_registry() -> &'static HashMap<&'static str, BuiltinFn> {
    BUILTIN_REGISTRY.get_or_init(|| {
        ...
        m.insert("getPlatform", |a, s, sc, _| process::get_platform(a, s, sc));
        m.insert("getArch", |a, s, sc, _| process::get_arch(a, s, sc));
        m.insert("processExit", |a, s, sc, _| process::process_exit(a, s, sc));
        ...
        // NOTE: WebSocket entries (wsConnect, wsSend, etc.) are in the net namespace block above (B29)
