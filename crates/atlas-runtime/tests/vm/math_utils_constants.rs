...
fn test_clamp_within_range() {
    let result = execute_vm_ok("clamp(5, 1, 10);");
...
fn test_clamp_above_max() {
    let result = execute_vm_ok("clamp(15, 1, 10);");
    assert_eq!(result, Value::Number(10.0));
}

#[test]
fn test_clamp_at_min() {
    let result = execute_vm_ok("clamp(1, 1, 10);");
    assert_eq!(result, Value::Number(1.0));
}

#[test]
fn test_clamp_at_max() {
    let result = execute_vm_ok("clamp(10, 1, 10);");
    assert_eq!(result, Value::Number(10.0));
}

#[test]
fn test_clamp_invalid_range_returns_nan() {
    let result = execute_vm_ok("clamp(5, 10, 1);");
...
fn test_sign_positive() {
    let result = execute_vm_ok("sign(42);");
    assert_eq!(result, Value::Number(1.0));
}
...
#[test]
fn test_sign_zero() {
    let result = execute_vm_ok("sign(0);");
    assert_eq!(result, Value::Number(0.0));
}

#[test]
fn test_sign_nan() {
    let result = execute_vm_ok("sign(0 / 0);");
    assert!(matches!(result, Value::Number(x) if x.is_nan()));
}
...
fn test_random_in_range() {
    let result = execute_vm_ok("random();");
    match result {
        Value::Number(n) => {
...
fn test_constant_sqrt2() {
    let result = execute_vm_ok("SQRT2;");
    match result {
        Value::Number(n) => {
