...
fn test_format_reparse_complex(#[case] source: &str) {
    ...
    // Note: Idempotency check skipped — the formatter currently emits parenthesized
    // conditions (e.g. `if (cond)`), which triggers the no-paren parser warning on
    // re-format. This will be fixed when the formatter is updated for H-080.
}
