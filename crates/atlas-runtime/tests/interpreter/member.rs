...
fn test_json_as_string_error() {
    let result = run_interpreter(
        r#"let data: json = unwrap(Json.parse("{\"age\":30}")); data["age"].as_string();"#,
    );
...
fn test_json_as_bool_error() {
    let result = run_interpreter(
        r#"let data: json = unwrap(Json.parse("{\"count\":5}")); data["count"].as_bool();"#,
    );
    assert!(result.is_err());
...
fn test_as_string_on_null() {
    let result = run_interpreter(
        r#"let data: json = unwrap(Json.parse("{\"v\":null}")); data["v"].as_string();"#,
    );
    assert!(result.is_err());
}
...
fn test_as_number_on_null() {
    let result = run_interpreter(
        r#"let data: json = unwrap(Json.parse("{\"v\":null}")); data["v"].as_number();"#,
    );
    assert!(result.is_err());
}
...
fn test_as_bool_on_null() {
    let result = run_interpreter(
        r#"let data: json = unwrap(Json.parse("{\"v\":null}")); data["v"].as_bool();"#,
    );
    assert!(result.is_err());
}
