use super::normalize_path_for_test;
...
#[test]
fn test_resolve_prefers_atlas_extension() {
    let temp_dir = TempDir::new().unwrap();
    let root = temp_dir.path().to_path_buf();

    let main = root.join("main.atlas");
    let lib = root.join("lib.atlas");
    ...
    let resolved = resolver
        .resolve_path("./lib", &main, Span::dummy())
        .expect("Expected resolver to find .atlas module");
