...
#[case("fn f<T extends { as_string: () -> string }>(x: T) -> string { return x.as_string(); } let y: json = unwrap(parse_json(\"{}\")); let z = f(y);")]
#[case("fn f<T extends { value: json }>(x: T) -> T { return x; } let y: json = unwrap(parse_json(\"{}\")); let z = f(y);")]
...
#[case("fn f<T extends Equatable>(x: T) -> bool { return true; } let y: json = parse_json(\"{}\"); let z = f(y);")]
...
#[case("fn f<T extends Equatable>(x: T) -> T { return x; } let y: json = parse_json(\"{}\"); let z = f(y);")]
