...
#[case("type Pair<T, U> = (T, U): T; fn fst<T, U>(own x: T, borrow _y: U): T { return x; } let _x: Pair<number, string> = fst;")]
...
// ResultMap with wrong value type should produce a type mismatch error
#[test]
fn test_alias_declarations_result_map_type_mismatch() {
    let diags = errors("type ResultMap = Map<string, Result<number, string>>; let _x: ResultMap = new Map<string, number>();");
    assert!(
        !diags.is_empty(),
        "Expected type mismatch error for Map<string, number> assigned to ResultMap"
    );
}
...
#[case("type A = (number): number; fn f(own x: number): number { return x; } let _x: A = f;")]
...
#[case("type Pair<A, B> = (A, B): A; fn fst<A, B>(own a: A, borrow _b: B): A { return a; } let _x: Pair<number, string> = fst;")]
#[case("type Pair<A, B> = (A, B): B; fn snd<A, B>(borrow _a: A, own b: B): B { return b; } let _x: Pair<number, string> = snd;")]
#[case("type MapEntry<K, V> = (K, V): V; fn pick<K, V>(borrow _k: K, own v: V): V { return v; } let _x: MapEntry<string, number> = pick;")]
...
#[case("type A = number[]; type B = number[]; let _x: A = [1]; let _y: B = _x;")]
#[case("type A = (number): number; type B = (number): number; fn f(own x: number): number { return x; } let _x: A = f; let _y: B = _x;")]
...
#[rstest]
#[case("type UserId = string; fn f(own id: UserId): UserId { return id; }")]
#[case("type Count = number; fn f(own x: Count): number { return x; }")]
