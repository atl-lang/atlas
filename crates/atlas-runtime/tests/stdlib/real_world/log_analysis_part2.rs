...
fn test_log_extract_ip_addresses() {
    ...
    let code = format!(
        ...
        let logs: string = file.read("{}").unwrap();
...
fn test_log_group_by_category() {
    ...
    let code = format!(
        ...
        fn isDatabase(borrow line: string): bool {{
            return line.startsWith("DB:");
        }}

        let logs: string = file.read("{}").unwrap();
        let lines: string[] = logs.split("\n");
        let dataLines: string[] = lines.slice(0.0, len(lines) - 1.0);
...
fn test_log_parse_structured() {
    ...
    let code = format!(
        ...
        levelPart.startsWith("level=error")
    "#,
        path_for_atlas(&log_path)
    );
...
fn test_log_reverse_chronological() {
    ...
    let code = format!(
        ...
        let lines: string[] = logs.split("\n");
        let dataLines: string[] = lines.slice(0.0, len(lines) - 1.0);
        let reversed: string[] = dataLines.reverse();
        reversed[0]
    "#,
        path_for_atlas(&log_path)
    );
...
fn test_log_summary_report() {
    ...
    let code = format!(
        r#"
        fn isError(borrow line: string): bool {{ return line.includes("ERROR"); }}
        fn isWarn(borrow line: string): bool {{ return line.includes("WARN"); }}
        fn isInfo(borrow line: string): bool {{ return line.includes("INFO"); }}
...
fn test_log_filter_time_range() {
    ...
    let code = format!(
        ...
        fn isMorning(borrow line: string): bool {{
            let time: string = line.substring(0.0, 2.0);
...
fn test_log_parse_json_lines() {
    ...
    let code = format!(
        ...
        let level: string = json["level"].asString();
        level
    "#,
        path_for_atlas(&log_path)
    );
...
fn test_log_aggregate_metrics() {
    ...
    let code = format!(
        ...
        fn sumLatency(borrow total: number, borrow line: string): number {{
            let parts: string[] = line.split(":");
            let value: number = unwrap(parts[1].toNumber());
            return total + value;
        }}
...
fn test_log_combine_multiline() {
    ...
    let code = format!(
        ...
        combined.includes("Start") && combined.includes("Continue") && combined.includes("End")
    "#,
        path_for_atlas(&log_path)
    );
    assert_eval_bool_with_io(&code, true);
