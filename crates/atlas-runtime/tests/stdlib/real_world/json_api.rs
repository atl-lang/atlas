...
fn test_json_parse_simple_object() {
    let code = r#"
        ...
        let data: json = unwrap(parse_json(jsonStr));
...
fn test_json_parse_array() {
    let code = r#"
        ...
        let arr: json = unwrap(parse_json(jsonStr));
...
fn test_json_api_extract_users() {
    let code = r#"
        ...
        let response: json = unwrap(parse_json(jsonStr));
...
fn test_json_build_from_parts() {
    let code = r#"
        ...
        let parsed: json = unwrap(parse_json(jsonStr));
...
fn test_json_prettify_output() {
    let code = r#"
        let jsonStr: string = "{\"name\":\"Alice\",\"age\":30}";
        let data: json = unwrap(parse_json(jsonStr));
        let pretty: string = prettify_json(jsonStr, 2.0);
        includes(pretty, "  ")
    "#;
    assert_eval_bool_with_io(code, true);
}
...
fn test_json_validate_before_parse() {
    let code = r#"
        ...
        let valid: bool = is_valid_json(validJson);
        let invalid: bool = is_valid_json(invalidJson);
        valid && !invalid
    "#;
    assert_eval_bool_with_io(code, true);
}
...
fn test_json_github_api_style() {
    let code = r#"
        ...
        let json: json = unwrap(parse_json(response));
...
fn test_json_minify_compact() {
    let code = r#"
        let jsonStr: string = "{  \"name\" :  \"test\"  }";
        let minified: string = minify_json(jsonStr);
        !includes(minified, "  ")
    "#;
    assert_eval_bool_with_io(code, true);
}
