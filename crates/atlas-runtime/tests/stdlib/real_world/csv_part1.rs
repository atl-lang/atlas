...
fn test_csv_parse_rows() {
    ...
    let code = format!(
        ...
        let lines: string[] = csv.split("\n");
        let header: string = lines[0];
        let dataLines: string[] = lines.slice(1, len(lines));
...
fn test_csv_filter_by_criteria() {
    ...
    let code = format!(
        ...
        let csv: string = file.read("{}").unwrap();
        let lines: string[] = csv.split("\n");
        let dataLines: string[] = lines.slice(1.0, len(lines) - 1.0);
...
fn test_csv_write_transformed() {
    ...
    let code = format!(
        ...
        fn transform(borrow row: string): string {{
            let fields: string[] = row.split(",");
            let name: string = fields[0];
            let value: number = unwrap(fields[1].toNumber());
...
fn test_csv_filter_and_count() {
    ...
    let code = format!(
        ...
        fn isAdult(borrow row: string): bool {{
            let fields: string[] = row.split(",");
            let age: number = unwrap(fields[1].toNumber());
            return age >= 30.0;
        }}
