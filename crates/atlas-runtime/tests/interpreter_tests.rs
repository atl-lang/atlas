...
fn test_numeric_overflow_multiplication() {
    let runtime = Atlas::new();
    let code = r#"
        let huge1: number = 1e308;
        let huge2: number = 2.0;
...
fn test_numeric_very_large_division() {
    let runtime = Atlas::new();
    let code = r#"
        let huge: number = 1e308;
        let tiny: number = 1e-308;
...
fn test_numeric_valid_large_numbers() {
    ...
    // This should work - 3e50 is still within valid range
    match runtime.eval(code) {
        Ok(Value::Number(n)) => {
            assert!(n > 0.0);
            assert!(n.is_finite());
