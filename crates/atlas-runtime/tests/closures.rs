...
fn test_parameter_does_not_bleed_to_outer_scope() {
    ...
    assert_parity_number(
        r#"
let mut x = 99;
fn set_inner(borrow y: number) { let mut x = y; }
set_inner(1);
x;
"#,
        99.0,
...
fn test_tc_anon_fn_arrow_syntax_no_error() {
    ...
    typecheck_ok(
        r#"
let double = fn(x: number): number { x };
double(4);
"#,
    );
}
...
fn test_anon_fn_arrow_form_basic() {
    // Phase 05 AC: `let f = fn(x: number): number { x * 2 }; f(3);` → 6
    assert_vm_number(
        r#"
...
fn test_anon_fn_arrow_form_two_params() {
    assert_vm_number(
        r#"
let mul = fn(a: number, b: number): number { a * b };
mul(6, 7);
"#,
        42.0,
    );
...
fn test_anon_fn_arrow_form_string_concat() {
    assert_vm_string(
        r#"
let join = fn(a: number, b: number): number { a + b };
join("foo", "bar");
"#,
        "foobar",
    );
...
fn compute(borrow a: number, borrow b: number): number {
    let f = fn(x: number): number { x + a + b };
    return f(1);
}
compute(2, 3);
"#,
...
apply(fn(n: number): number { n * 3 }, 4);
"#,
        12.0,
    );
}
...
apply(fn(n: number): number { n + 100 }, 5);
"#,
        105.0,
    );
}
...
fn make_multiplier(borrow factor: number): any {
    return fn(x: number): number { x * factor };
}
...
fn test_hof_map_arrow() {
    assert_parity_number(
        r#"
let arr = [1, 2, 3];
let result = map(arr, fn(x: number): number { x * 2 });
result[0];
"#,
        2.0,
    );
...
fn run(): number {
    let factor = 3;
    let arr = [1, 2, 3];
    let result = map(arr, fn(x: number): number { x * factor });
    return result[1];
}
run();
"#,
...
fn test_hof_filter_arrow() {
    assert_parity_number(
        r#"
let arr = [1, 2, 3, 4, 5];
let result = filter(arr, fn(x: number): bool { x > 1 });
len(result);
"#,
        4.0,
    );
...
fn run(): number {
    let threshold = 2;
    let arr = [1, 2, 3, 4];
    let result = filter(arr, fn(x: number): bool { x > threshold });
    return len(result);
}
run();
"#,
...
fn test_hof_reduce_arrow() {
    assert_parity_number(
        r#"
let arr = [1, 2, 3, 4];
reduce(arr, fn(acc: number, x: number): number { acc + x }, 0);
"#,
        10.0,
    );
}
...
fn test_hof_for_each_fn_expr_executes() {
    ...
    let result = vm_eval(
        r#"
let arr = [1, 2, 3];
for_each(arr, fn(borrow x: number) { });
42;
"#,
    );
...
fn test_hof_for_each_arrow_executes() {
    let result = vm_eval(
        r#"
let arr = [1, 2, 3];
for_each(arr, fn(x: number): number { x });
99;
"#,
    );
...
fn test_hof_any_arrow() {
    assert_parity_bool(
        r#"
let arr = [1, 2, 3];
some(arr, fn(x: number): bool { x > 10 });
"#,
        false,
    );
}
...
fn test_hof_all_arrow() {
    assert_parity_bool(
        r#"
let arr = [1, 2, 3];
every(arr, fn(x: number): bool { x > 1 });
"#,
        false,
    );
}
...
fn test_hof_sort_arrow() {
    assert_parity_number(
        r#"
let arr = [3, 1, 2];
let result = sort(arr, fn(a: number, b: number): number { a - b });
result[2];
"#,
        3.0,
    );
...
fn test_hof_flat_map_arrow() {
    assert_parity_number(
        r#"
let arr = [1, 2, 3];
let result = flat_map(arr, fn(x: number): number[] { [x, x] });
result[0];
"#,
        1.0,
    );
